...
* 数组和集合的区别
	* 区别1 : 
		...
		* 集合只能存储引用数据类型(对象)集合中也可以存储基本数据类型,但是在**存储的时候会自动装箱变成对象**
	* 区别2:
		* 数组长度是固定，初始化后就不能修改。
		* 集合的长度的是可变的,可以根据元素的增加而增长（本质上还是数组实现的，变长的时候创建新数组丢弃旧数组）
* 什么时候用数组，什么时候用集合
		* 1,如果元素个数是固定的推荐用数组
		* 2,如果元素个数不是固定的推荐用集合
* 集合继承体系图
	![集合框架](./collection.png)
	* collection[接口] 下分为List和Set
	* list：代表列表，代表有序！里面的元素按照存入的次序排列，数据是可以重复的。
		* LinkedList是链表
		* ArrayList和Vector是数组 
	* set：代表集合，代表无序！
		* HashSet是hash算法
