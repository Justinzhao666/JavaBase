...
* 异常的分类
	* 通过API查看Throwable有两个子类：Error，Exception；
	* Error：
		* 服务器宕机,数据库崩溃等
	* Exception分为两种异常：
		*  运行时异常：程序运行中可能出现的异常，需要我们自己自觉的处理！程序出错，只有在执行程序的时候才能发现！
		* 编译时异常：除了运行时异常之外的所有异常，在编写代码的时候，代码检查阶段就需要进行处理！
* Exception异常处理方式：
	* 自己处理：try catch
	* 将异常交给上级处理：throwable
* 异常的继承体系
	* Throwable
		* Error	
		* Exception
			* RuntimeException：程序员自己
			* 其他非运行时异常：在代码检查阶段就需要处理。
